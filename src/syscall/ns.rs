...
use glenda::interface::{CSpaceService, FileSystemService, VirtualFileSystemService};
...
impl<'a, 'b> UserAccessSession<'a, 'b> {
    ...
    pub fn sys_mount(&mut self, sp: usize) -> Result<usize, Error> {
        ...
        let old = self.strncpy_from_user(old_ptr, 1024)?;
        let aname = if aname_ptr != 0 {
            self.strncpy_from_user(aname_ptr, 1024)?
        } else {
            String::from("")
        };
    ...
    pub fn sys_unmount(&mut self, sp: usize) -> Result<usize, Error> {
        ...
        let old =
            if old_ptr != 0 { self.strncpy_from_user(old_ptr, 1024)? } else { String::from("") };
