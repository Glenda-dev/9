...
use crate::layout::{ARM64_ALIGN, USTKSIZE, USTKTOP, UTZERO};
...
impl<'a> NineManager<'a> {
    pub fn exec_p9_binary(
        ...
        log!(
            "Nine: Loading a.out binary: text={:#x}@{:#x}, data={:#x}@{:#x}, bss={:#x}@{:#x}",
            text_size,
            text_base,
            data_size,
            data_base,
        ...
        if data_size > 0 {
            self.map_segment(
                pid,
                data_base,
                &data[(32 + text_size)..(32 + text_size + data_size)],
    ...
    fn setup_p9_stack(&mut self, pid: usize, argv: &[String]) -> Result<usize, Error> {
        ...
        let scratch = self.vspace_mgr.map_scratch(
            frame,
            Perms::READ | Perms::WRITE,
            1,
            &mut *self.res_client,
        ...
        let tos = Tos { pid: pid as u32, ..Default::default() };
        unsafe {
